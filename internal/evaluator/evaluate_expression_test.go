...
func TestOperationErrors(t *testing.T) {
	testcases := []struct {
		...
		// Function call errors
		{
			...
			input:        "SUM(true)",
			expectsError: "SUM(values:number...):number received invalid argument <bool true> in (SUM <bool true>), at <IDENT:SUM test:1:1>",
		},
		{
			name:         "SUM with mixed incompatible types in integer sum",
			input:        "SUM(5, \"hello\")",
			expectsError: "SUM(values:number...):number received invalid argument <str \"hello\"> in (SUM <int 5> <str \"hello\">), at <IDENT:SUM test:1:1>",
		},
		{
			name:         "SUM with mixed incompatible types in float sum",
			input:        "SUM(5.5, true)",
			expectsError: "SUM(values:number...):number received invalid argument <bool true> in (SUM <float 5.50> <bool true>), at <IDENT:SUM test:1:1>",
		},
		{
			name:         "SUM with mixed incompatible types in string sum",
			input:        "SUM(\"hello\", 42)",
			expectsError: "SUM(values:number...):number received invalid argument <str \"hello\"> in (SUM <str \"hello\"> <int 42>), at <IDENT:SUM test:1:1>",
		},
	}
