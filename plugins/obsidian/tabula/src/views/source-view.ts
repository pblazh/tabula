...
export class SourceView extends TextFileView {
  ...
  private async addEditorStyles() {
    try {
      // Load CSS content from file (for now, inline as backup)
      const cssContent = `
...
.csv-source-cm-container .cm-lineNumbers .cm-gutterElement {
  color: var(--text-faint);
      ...
      style.id = "csv-source-view-styles";
      ...
      console.warn('Failed to load CSS styles:', error);
    }
  }
