import { parseCSVContent, unparseCSVContent, validateCSVSettings } from '../../services/csv-service';
...
describe('CSV Service', () => {
  ...
  describe('validateCSVSettings', () => {
    ...
    it('should detect delimiter same as quote', () => {
      ...
      expect(errors).toContain('Delimiter and quote character cannot be the same');
    });
    ...
    it('should detect delimiter same as comment prefix', () => {
      ...
      expect(errors).toContain('Delimiter and comment prefix cannot be the same');
    });
  ...
  describe('Integration tests', () => {
    it('should round-trip CSV data correctly', () => {
      const originalCSV = '# Comment\nname,age,city\nJohn,25,"New York, NY"\nJane,30,LA';
      
      // Parse the CSV
      const parsed = parseCSVContent(defaultSettings, originalCSV);
      
      // Unparse it back
      const unparsed = unparseCSVContent(defaultSettings, parsed.data, parsed.comments);
